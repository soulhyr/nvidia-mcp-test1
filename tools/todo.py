...
def register_todo_tools(mcp):
    ...
    @mcp.tool()
    async def list_todos(assignee: Optional[str] = None, status: Optional[str] = None) -> str:
        """할 일을 조회합니다. assignee(담당자)나 status('completed'/'pending')로 필터링 가능."""
        todos = load_todos()
        if assignee:
            todos = [t for t in todos if t.get("assignee") == assignee]
        if status == "completed":
            todos = [t for t in todos if t.get("completed") is True]
        elif status == "pending":
            todos = [t for t in todos if t.get("completed") is False]
