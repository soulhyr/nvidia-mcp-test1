...
## 5. Claude Desktop 에서 실제 동작 해보기.
### 1. 초기화.
- prompt : 모든 데이터 초기화 해줘.
  => 등록된 사원 정보 및 할일 목록을 초기화 한다.

### 2. 사원 정보 등록.
- prompt : 클로드 데스크탑에서 _sample_data 의 emp.csv 파일을 첨부한다.
  => 각 인원별로 /data/employees/성명.md 파일이 생성된다.
  
### 3. 사원별 할일 등록.
- prompt : XXX 님 내일 강남 미팅 잡아줘.
  => XXX 의 할일 목록에 강남 미팅이 등록된다.
- prompt : OOO 님 이번주 금요일 부산 미팅 잡아줘.
  => OOO 의 할일 목록에 부산 미팅이 등록된다.
  
### 4. 사원 정보 조회.
- prompt : XXX 사원 정보 조회해줘.
=> XXX 의 사원 정보가 프롬프트에 등록한 양식에 맞게 조회된다.
